---
name: postgresql
description: "Designs and reviews PostgreSQL schemas for the cinemauthor platform — data types, primary keys, FK indexes, constraints, partitioning, RLS, JSONB, and safe DDL migrations. Use this skill even when 'PostgreSQL' or 'schema' are not mentioned explicitly — if the user says 'design the database table', 'which data type should I use', 'add an index', 'set up row-level security', 'review this migration', 'plan partitioning for this table', or 'should I use JSONB or a column here', this skill applies. Does NOT apply to query tuning without schema changes, non-PostgreSQL databases, TypeORM entity class design (use nestjs-expert), or DB-agnostic ORM modeling."
metadata:
  risk: unknown
  source: community
---
