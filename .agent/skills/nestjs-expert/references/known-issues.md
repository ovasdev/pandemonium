# Known NestJS Issues — Real Cases from GitHub & Stack Overflow

Reference for problem-specific debugging. Load this file when the user reports an error message matching one of the headings below.
...
7. [secretOrPrivateKey must have a value]
...
13. [NestJS DI error messages are generic]
...
1. Check GitHub issues for the affected NestJS version
2. Try downgrading to the previous stable version
3. Update to the latest patch version
4. Report regressions with a minimal reproduction
...
3. Add custom error messages in module providers
4. Use `nest info` and module-level logging to narrow down the injection chain
